...
*** Use cases:
**** Message arrives when user not logged in
***** Seq number is current
***** Seq number is not current

**** Message arrived when user session is initialized

*** SYNC protocol use cases:
**** Private message sync:
1. Island receives message with seq number which is greater than last recorded
   by more than one, meaning that some messages are skipped
   - Place message in processing queue
   - Send last seq seen from the member
   - if member responses with missed messages:
...
Counters:
   outgoing:
      public,
      private for each member

   incoming:
      public and private for each member
...
*** TODO get rid if simple RAM queue
*** TODO Expiration time for messages

** TODO improve server logging. Implement Logging categories and runtime rule changes.
* Bugs
