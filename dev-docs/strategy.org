...
*** TODO Re-design client server protocol. Remove request-response stuff
*** TODO Vault id should be pkfp
*** TODO On establish connection launch all hidden services and topic authorities async
*** TODO Logger config parser: change format and implement self-documenting file
*** TODO Figure out socket.io connection bugs and dead sockets
*** TODO Session now keeps vault ID
*** TODO Query parameters on handshake
Vault id, topics ID
*** TODO Take out holding topic from vault to chat
...
*** PROJ Improve socket protocol. Remove requests as they are unneeded
