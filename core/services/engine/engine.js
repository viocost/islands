...
const { removeItem, copy } = require("./util")
...
/**
 * If more global dependencies will have to be linked - this will need to be refactored
 * */
function linkGlobalChatLibraries(){
    let globalLibraries = [ 'better-sqlite3' ]

    for( let lib of globalLibraries ){
        let localNodeModules = path.join(process.env["APPS"], "chat", "node_modules")
        let linkPath =  path.join(localNodeModules,  lib)
        let libPath = path.join(process.env["NODE_MODULES"], lib)

        if(!fs.existsSync(libPath)){
            throw new Error(`Missing global dependency: ${lib}`)
        }
        ...
        // TODO Refactor
        // this is dirty hack to make windows work without asking permission
        if(platform() === "win32"){
            copy(libPath, localNodeModules)
        }else {
            fs.symlinkSync(libPath, linkPath)
        }
    }
}
...
async function launchChat(){
    ...
    console.log("linking global dependencies...");
    linkGlobalChatLibraries();
...
//Returns relevant core subdirectory name based on platform
function coreSubdir(){
    let subdirMap = {
        linux: "linux",
        darwin: "mac",
        "win32": arch() === "x64" ? "win64" : "win32"
    }

    return subdirMap[platform()]
}
...
function initLinux(){
    ...
    process.env["NODE_MODULES"] = path.join(process.env["BASE"], "core/linux/lib/node_modules");
...
function initMac(){
    ...
    process.env["NODE_MODULES"] = path.join(process.env["BASE"], "core/mac/lib/node_modules");
...
function initWin(){
    ...
    process.env["NODE_MODULES"] = path.join(process.env["BASE"], "core", winPath, "node", "node_modules");
...
function printVersion(){
    ...
    try{
        ...
        let coreVersion = fs.readFileSync(path.join(process.env["BASE"], "core", coreSubdir(), "core.version"), "utf8")
...
`
        ...
        console.log(`Error printing version ${err}`);
    }
    console.log("Exiting...")
    process.exit(0)
