...
export class Vault{
    ...
    //This property is needed for temporary storing version 1 topic keys
    v1Topics;
    constructor(version){
    ...
    set password(password){
        if(typeof password === "string"){
            this._password = new SecretHandle(password, "Vault password hidden")
        } else if ( password instanceof SecretHandle ){
            this._password = password
        } else {
            throw new TypeError(`Password type is invalid: ${typeof password}`)
        }
    }

    get password(){
        if(this._password instanceof SecretHandle){
            return this._password.getSecret();
        }
        return ""
    }
    ...
    deleteTopic(pkfp){
        ...
        // CRYPTO:  Sign delete topic request
        let ic = new iCrypto();
        ic.createNonce("n")
            .bytesToHex("n", "nhex")
            .setRSAKey("priv", this.privateKey, "private")
    ...
    initSaved(vaultRaw = Err.required("Vault parse: vaultRaw parameter missing")){
        ...
        if (!vaultRaw.version || semver.lt(vaultRaw.version, "2.0.0")){
            ...
            console.log(`vault format update required to version ${IslandsVersion.getVersion()}`)


            this.v1Topics = vaultRaw.topics;

            //////////////////////////////////////////////////
            // let self = this;                             //
            // this.version = IslandsVersion.getVersion();  //
    ...
    addNewTopic(self, data){
        ...
        newTopic.applySettings(metadata.body.settings)
...
export const VaultEvents = {
    ...
    VAULT_FORMAT_UPDATED: Symbol("vault_format_updated")
}
