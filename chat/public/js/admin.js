...
function signUpdateFile(filedata){
    let ic = new iCrypto();
    ic.setRSAKey("pk", adminSession.privateKey, "private")
        .addBlob("f", filedata)
        .privateKeySign("f", "pk", "sign");
    return ic.get("sign");
}
...
function requestLoggerStateChange(ev){
    ...
    $.ajax({
        ...
        success:  ()=>{
            let message = "Logger has been successfully " +
                (selectedElement.value === "true" ? "enabled" : "disabled");
            toastr.info(message);
        },
        err: (err)=>{
            toastr.warning("Error loading logs: " + err);
...
function filterLogs(ev){
    ...
    for (let i=0;i<logTableBody.childNodes.length;i++){
        ...
        filter.test(testingField) ? logTableBody.childNodes[i].classList.remove("log-row-hidden") :
            logTableBody.childNodes[i].classList.add("log-row-hidden")
    }
}
