...
function setupChatListeners(chat){
    ...
    chat.on("login_success", (messages)=>{
        document.querySelector('#sounds-switch').src = chat.session.settings.soundsOn ?
            soundsOnOfIcons.on : soundsOnOfIcons.off;
        loadingOff();
        clearAllInputs();
...
function sendMessage(){
    ...
    if(addressee === "ALL"){
        chat.shoutMessage(message.value.trim(), attachments)
            .then(()=>{
        ...
        chat.whisperMessage(addressee, message.value.trim())
            .then(()=>{
...
function get_current_time(){
    let d = new Date();
    return padWithZeroes(2, d.getHours()) + ':'
        + padWithZeroes(2, d.getMinutes())
}
...
function getChatFormatedDate(timestamp){
    ...
    if (Math.floor((today - d)/1000) <= 64000){
        ...
        return DAYSOFWEEK[d.getDay()] + ", " + d.getMonth()
            + "/" + padWithZeroes(2, d.getDate())
            + " " + padWithZeroes(2, d.getHours())
            + ':' + padWithZeroes(2, d.getMinutes())
    }
}
...
function updateLoadedMessages(){
    ...
    document.querySelector("#chat_window").childNodes.forEach(msg=>{
        if(msg.classList.contains("service-record")){
            ...
            if(!msg.classList.contains("private-message")){return}
            try{
...
function processLogin(messages){
    ...
    if (chat.session.metadata.topicName)
    document.title =  chat.session.metadata.topicName;
    updateParticipants();
    setNavbarListeners();
    syncPendingInvites();
...
function onLoginLoadMessages(messages){
    ...
    for (let i=0; i<messages.length; ++i){
        let message;
        try{
            message = typeof(messages[messages.length-i-1]) === "string" ?
                JSON.parse(messages[messages.length-i-1]):
        ...
        const alias = isMyMessage(pkfp) ? chat.getMemberNickname(pkfp) :
            chat.getMemberRepr(pkfp);
...
function processMessageBody(text){
    ...
    let substrings = text.split(startPattern).filter((el)=> {return el.length !== 0});
    for (let i=0; i<substrings.length; ++i){
        ...
        if (endCode === -1){
            ...
            if (rawAfterText.length>0)
                afterText = document.createTextNode(rawAfterText)
        }
        //highliter:
...
function syncPendingInvites(){
    ...
    for (let i in invites){
        ...
        inviteRep.value = chat.session.settings.invites[invites[i]].name ?
            chat.session.settings.invites[invites[i]].name : "New member";
